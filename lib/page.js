...
class Page extends Item {
  ...
  constructor(file, options, parent) {
    ...
    this.identifier = path.relative(options.paths.content, path.dirname(this.file)).replace(/(\d+)-([^/]+)/gi, '$2');
    this.url = `${options.base}/${options.permalink(this.identifier)}`;
    ...
    this.basepath = (url = '') => `${options.base}${url}`;
